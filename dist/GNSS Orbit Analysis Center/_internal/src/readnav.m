function [iono_coeff, eph] = readnav(ephemerisfile)

% This function reads a RINEX GPS Navigation Message file and reformats the
% data into a matrix with 28 rows and a column for each satellite. 
% 
% Input :  ephemerisfile    - string with path of RINEX navigation file
% 
% Output : iono_coeff       - ionosphere coefficients for Klobuchar model
%          eph:             - matrix with data from rinex-file. Units are either seconds, meters, or radians. 
...
for i = 1:noeph
    ...
    jd = julday(year,month,day,hour);           % Jul. day of clock
    [~, toc] = gps_time(jd);                    % ~, seconds of gps-week
